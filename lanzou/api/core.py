...
class LanZouCloud(object):
    ...
    def recovery_all(self) -> int:
        ...
        if not first_page:
            return LanZouCloud.NETWORK_ERROR
        post_data["formhash"] = re.findall(
            r'name="formhash" value="(.+?)"', first_page.text
        )[
            0
        ]  # 设置表单 hash
            ...
            for file in resp["text"]:
                file_list.append(
                    File(
                        ...
                        has_pwd=(
                            True if int(file["onof"]) == 1 else False
                        ),  # 是否存在提取码
                        has_des=(
                            True if int(file["is_des"]) == 1 else False
                        ),  # 是否存在描述
                    )
                )
        return file_list
    ...
    def get_dir_list(self, folder_id=-1) -> FolderList:
        ...
        resp = self._post(
            self._doupload_url + "?uid=" + str(self._uid), post_data
        )  # 上传文件时需要 uid 参数
        if not resp:
            return folder_list
    ...
    def get_file_info_by_url(self, share_url, pwd="") -> FileDetail:
        ...
        try:
            if (
                'id="pwdload"' in first_page or 'id="passwddiv"' in first_page
            ):  # 文件设置了提取码时
                if len(pwd) == 0:
                    return FileDetail(
                ...
                f_time = (
                    time_format(f_time.group(1)) if f_time else time_format("0 小时前")
                )
                ...
                f_name = (
                    ...
                    or re.search(
                        r'<div class="filethetext".+?>([^<>]+?)</div>', first_page
                    )
                    or re.search(
                        r'<div style="font-size.+?>([^<>].+?)</div>', first_page
                    )
                    or re.search(r"var filename = '(.+?)';", first_page)
                    or re.search(r'id="filenajax">(.+?)</div>', first_page)
                    or re.search(
                        r'<div class="b"><span>([^<>]+?)</span></div>', first_page
                    )
                )
                f_name = (
                    f_name.group(1).replace("*", "_") if f_name else "未匹配到文件名"
                )
                ...
                if "?webpage=" in share_url:
                    ...
                    web_sign = re.search(
                        r"var a?websigna?\s*=\s*'(.+?)';", first_page
                    ).group(1)
                    web_sign_key = re.search(
                        r"var c?websignkeyc?\s*=\s*'(.+?)';", first_page
                    ).group(1)
                    post_data = {
                        "action": "downprocess",
                        "signs": ajax_data,
    ...
    def _upload_small_file(
        self, file_path, folder_id=-1, *, callback=None, uploaded_handler=None
    ) -> int:
    ...
    def _upload_big_file(
        self, file_path, dir_id, *, callback=None, uploaded_handler=None
    ):
        ...
        def _callback(name, t_size, now_size):  # 重新封装回调函数，隐藏数据块上传细节
            ...
            if callback is not None:
                ...
                now_size = (
                    now_size if now_size < file_size else file_size
                )  # 99.99% -> 100.00%
        ...
        while uploaded_size < file_size:
            ...
            code = self._upload_small_file(
                data_path, dir_id, callback=_callback, uploaded_handler=_close_pwd
            )
        ...
        code = self._upload_small_file(
            record_file_new, dir_id, uploaded_handler=_close_pwd
        )  # 上传记录文件
        if code != LanZouCloud.SUCCESS:
    ...
    def upload_file(
        ...
        return self._upload_big_file(
            file_path, dir_id, callback=callback, uploaded_handler=uploaded_handler
        )
    ...
    def get_folder_info_by_url(self, share_url, dir_pwd="") -> FolderDetail:
        ...
        try:
            ...
            folder_time = re.findall(
                r'class="rets">([\d\-]+?)<a', html
            )  # ['%m-%d'] 或者 None (vip自定义)
            folder_time = (
                folder_time[0] if folder_time else datetime.today().strftime("%m-%d")
            )  # 没有就设为现在
            folder_desc = re.findall(r'id="filename">(.+?)</span>', html) or re.findall(
        ...
        for url, name, desc in all_sub_folders:
            ...
            time_str = datetime.today().strftime(
                "%Y-%m-%d"
            )  # 网页没有时间信息, 设置为今天
            sub_folders.append(
                FolderInfo(name=name, desc=desc, url=url, time=time_str, pwd=dir_pwd)
            )
